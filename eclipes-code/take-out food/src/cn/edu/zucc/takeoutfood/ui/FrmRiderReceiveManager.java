...
public class FrmRiderReceiveManager extends JDialog implements ActionListener {
		private JPanel toolBar = new JPanel();
		private Button btnDelete = new Button("接订单");
		...
		public FrmRiderReceiveManager(FrmMain f, String s, boolean b) {
			super(f, s, b);
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(e.getSource()==this.btnDelete){
				...
				if(JOptionPane.showConfirmDialog(this,"确定接该订单吗？","确认",JOptionPane.YES_NO_OPTION)==JOptionPane.YES_OPTION){
					int Orderid=Integer.parseInt(this.tblData[i][0].toString());
					try {
						(new RiderSendManager()).addOrder(Orderid);
						this.reloadUserTable();
